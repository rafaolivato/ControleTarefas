...
export default class ItemDatabase {
                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM Tarefa LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS Tarefa (id INTEGER PRIMARY KEY AUTOINCREMENT, descricao varchar(30), data varchar(30), prioridade varchar(30))');
                        }).then(() => {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM Tarefa', []).then(([tx, results]) => {
                        ...
                        for (let i = 0; i < len; i++) {
                            ...
                            const { id, descricao, data, prioridade } = row;
                            lista.push({ id, descricao, data, prioridade });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
    ...
    Inserir(tarefa) {  //**** CRUD => UPDATE - aqui um registro da tabela é inserido *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para inserir um novo registro 
                    tx.executeSql('INSERT INTO Tarefa (descricao, data, prioridade) VALUES (?, ?, ?)', [tarefa.descricao, tarefa.data, tarefa.prioridade]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
    ...
    Atualizar(tarefa) {   //**** CRUD => UPDATE - aqui a tabela é atualizada *****/
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    //Query SQL para atualizar um registro no banco        
                    tx.executeSql('UPDATE Tarefa SET prioridade = ? WHERE id = ?', ["Finalizada", tarefa.id]).then(([tx, results]) => {
                        resolve(results);
